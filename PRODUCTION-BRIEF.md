...
## 十一、Higgsfield Academy 官方教程（补充）

从 Academy 页面发现，Higgsfield 有系统的官方教程：

### 电影制作类课程
1. **Blockbuster 4K: The AI Filmmaking Pipeline** — 40分钟，10个模块，覆盖脚本→资产创建→逐场景提示词工程
2. **Build an Ultra-Realistic Short Film in 4K** — 33分钟，19个模块，用 Claude Fable 5 + Seedance 2.0 4K 逐镜构建足球短片
3. **Add AI VFX to Real Footage** — 12分钟，11个模块，用 Seedance 2.0 给实拍素材加 VFX
4. **Make an AI Animated Short** — 17分钟，11个模块，跨8个视觉世界构建动画短片

### Prompt Bank（提示词库）
- **Camera 分类：** 46个摄像机运镜示例（Static shot、Pan right 等），每个都带完整提示词和效果演示
- 可直接复制使用

### Seedance 2.5 产品特性（官方）
- 单次生成最长30秒
- 原生4K输出
- 同步音频生成（环境音、拟音、配乐与画面同步）
- 最多50个多模态参考输入（图片+视频）
- 区域级编辑（不重新生成整段，只修局部）
- 角色、服装、光线跨镜头一致性
- 宽高比从9:16竖屏到21:9宽屏
